...
const createUser = asyncHandler(async (req: Request<{}, {}, RegisterBody>, res: Response) => {
  ...
  try {
    await newUser.save();
    generateToken(res, newUser._id as string);
...
const loginUser = asyncHandler(async (req: Request<{}, {}, LoginBody>, res: Response) => {
  ...
  if (existingUser) {
    ...
    if (isPasswordValid) {
      generateToken(res, existingUser._id as string);
...
const getCurrentUserProfile = asyncHandler(async (req: AuthRequest, res: Response) => {
...
const updateCurrentUserProfile = asyncHandler(async (req: AuthRequest, res: Response) => {
  const user = await User.findById(req.user?._id);
